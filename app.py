...
def load_data(ticker, period='2y'):
    """Carica i dati storici per un dato ticker."""
...
if not spx_data.empty and not vix_data.empty:
    ...
    # Estrai valori più recenti per la logica
    latest_spx = spx_data['Close'].iloc[-1]
    latest_vix = vix_data['Close'].iloc[-1]
    latest_sma90 = spx_data['SMA90'].iloc[-1]
    latest_sma125 = spx_data['SMA125'].iloc[-1]
    latest_sma150 = spx_data['SMA150'].iloc[-1]

    # --- LAYOUT DASHBOARD ---
    col1, col2 = st.columns([3, 1]) # Colonna sinistra più grande

    with col1:
        # --- GRAFICO SPX CON MEDIE MOBILI ---
